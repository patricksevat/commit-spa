...
export const CommitDetailCard: FunctionComponent = () => {
  ...
  return (
    <Card className="detail-card" data-test="detail-card">
      <CardHeader
        title={author}
        subheader={date}
        avatar={<Avatar alt={author} src={avatar} />}
        action={
          <IconButton aria-label="settings" onClick={handleIconButtonClick} data-test="external-link">
            <OpenInNew />
          </IconButton>
        }
      />
      <CardContent>
        ...
        <Typography variant="caption">SHA: {selectedCommit?.sha}</Typography>
      </CardContent>
    </Card>
  )
}
