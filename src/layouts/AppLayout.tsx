  ...
  return (
    <>
      <AppBar position={'sticky'}>
        <Toolbar id="toolbar">
          {pathSegments[1] && (
            <IconButton edge="start" color="inherit" onClick={() => history.push('/')} data-test="back-button">
              <ArrowBack />
            </IconButton>
          )}
          <Typography variant="h6">{translate(`toolbarTitle/${pathSegments[1]}`)}</Typography>
          <Select id={'appbar__language-selector'} value={language} onChange={handleLanguageChange}>
            <MenuItem value={'en-us'}>
              <span role="img" aria-label="Select English">
                🇬🇧
              </span>
            </MenuItem>
